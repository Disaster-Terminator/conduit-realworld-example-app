...
function ensureBackendEnv() {
  if (existsSync("backend/.env")) {
    const current = readFileSync("backend/.env", "utf8");
    const repaired = current
      .replace(/^DEV_DB_PORT=5433\s*$/m, "DEV_DB_PORT=15433")
      .replace(/^DEV_DB_LOGGING=false\s*$/m, "DEV_DB_LOGGING=");
    if (repaired !== current) {
      writeFileSync("backend/.env", repaired);
      console.log("[demo] repaired backend/.env for local demo defaults");
    }
    console.log("[demo] backend/.env already exists; leaving it unchanged");
    return;
  }
  writeFileSync("backend/.env", backendEnv);
  console.log("[demo] wrote backend/.env for local Postgres on 127.0.0.1:15433");
}
...
await waitForPort({ host: "127.0.0.1", port: 15433, timeoutMs: 30000 });
