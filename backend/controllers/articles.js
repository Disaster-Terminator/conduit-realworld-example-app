...
const allArticles = async (req, res, next) => {
  try {
    ...
    // If status=draft is requested, only return drafts if the author matches loggedUser
    if (status === "draft") {
      if (
        !loggedUser ||
        !author ||
    ...
    if (favorited) {
      ...
      articles.count = await user.countFavorites(searchOptions);
    } else {
...
const updateArticle = async (req, res, next) => {
  try {
    ...
    // Prevent published→draft rollback
    if (article.status === "published" && status === "draft") {
      throw new ValidationError("Published articles cannot be reverted to draft");
    }
